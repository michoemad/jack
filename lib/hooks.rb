module Try
  ...
  class Utils
    ...
    def get_hollidays_between(earlier_date, later_date)
      hollidays = 0
      Rails.logger.info("get_hollidays_between: " + Try.get_hollidays.to_s)
      if Try.get_hollidays
        hollidays += Try.get_hollidays.count{|d| (earlier_date <= d.to_date && d.to_date <= later_date)}
      end
      return hollidays
    end
